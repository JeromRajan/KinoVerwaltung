<template>
  <div>
    <v-card
      ...
      width="100%"
    >
      <h1>{{ $t('Statistics.hallOccupancy') }}</h1>
      ...
      <v-select
        ...
        item-value="kinoId"
        label="Kino"
        @update:modelValue="loadHallOccupancy"
      >
      </v-select>
...
export default {
  ...
  data() {
    return {
      ...
      chartData: {
        labels: [], // Namen der Saale
        datasets: [
          {
            label: this.$t('Statistics.numberOfVisitors'),
            backgroundColor: '#f87979',
            data: [] // Anzahl der Besucher
          }
        ]
      },
      chartOptions: {
  ...
  methods: {
    ...
    async loadHallOccupancy() {
      this.isLoading = true
      const daten = await this.statisikService.getHallOccupancy(this.selectedCinema)
      if (daten) {
        this.chartData.labels = []
        this.chartData.datasets[0].data = []
        daten.forEach(element => {
          this.chartData.labels.push(element.name)
          this.chartData.datasets[0].data.push(element.anzahlBesucher)
        })
